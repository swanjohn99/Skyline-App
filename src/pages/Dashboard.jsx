...
export default function Dashboard() {
  ...
  const fetchData = useCallback(async () => {
    ...
    const [activeRes, totalRes, { data: projects }, { data: expenses }, { data: payments }] = await Promise.all([
      supabase.from('projects').select('*', { count: 'exact', head: true }).not('status', 'in', '("work ended","rejected")'),
    ...
    const completedJobs = projects.filter(p => p.status === 'work ended');
  ...
  return (
    <div className="page">
      <header className="page-header">
        ...
        <p className="page-subtitle">{currentMonthName} {currentYear} — overview of your construction projects</p>
      </header>
      ...
      <div className="metrics-grid">
        ...
        <MetricCard icon={TrendingUp} iconClass="green" label="Profit (Month)" value={`$${metrics.profitMonth.toLocaleString()}`} />
        <MetricCard icon={CalendarDays} iconClass="amber" label="Profit (Year)" value={`$${metrics.profitYear.toLocaleString()}`} />
      </div>
